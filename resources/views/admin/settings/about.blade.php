...
@section('content')
        ...
        <form action="{{ route('admin.about.update') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="col-lg-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="experience_years" class="form-control"
                                        value="{{ old('experience_years', $about->experience_years ?? '') }}">
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="mb-3">
                                    ...
                                    @if (!empty($about->hero_image))
                                        <img src="{{ asset($about->hero_image) }}" style="max-height:60px;" class="mt-2">
                                    @endif
                                </div>
                            </div>
                            <div class="col-lg-6">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="why_title" class="form-control"
                                        value="{{ old('why_title', $about->why_title ?? '') }}">
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="mb-3">
                            ...
                            <div class="col-lg-6">
                                <div class="mb-3">
                                    ...
                                    @if (!empty($about->why_choose_us_image))
                                        <img src="{{ asset($about->why_choose_us_image) }}" style="max-height:60px;"
                                            class="mt-2">
                                    @endif
                                </div>
                            </div>
                            <div class="col-lg-6">
                                        ...
                                        @if (count($whyList) > 0)
                                            @foreach ($whyList as $w)
                                                <div class="d-flex mb-2">
                                                    <input type="text" name="why_list[]" class="form-control me-2"
                                                        value="{{ $w }}">
                                                    <button type="button" class="btn remove-why">Remove</button>
                                                </div>
                                            @endforeach
                                        @else
                                            <div class="d-flex mb-2">
                                ...
                                <button type="button" id="add-why" class="btn btn-sm btn-secondary mt-2">Add
                                    Bullet</button>
                            </div>
                        </div>
            ...
            <div class="card mb-3">
                ...
                <div id="statsSection" class="section-body" style="display:none;">
                    <div class="card-body">
                        @php $stats = old('stats', $about->stats ?? [['label' => 'Happy Customers', 'number' => 15, 'suffix' => 'k'], ['label' => 'Team Members', 'number' => 15, 'suffix' => '+'], ['label' => 'Total Awards', 'number' => 50, 'suffix' => '+'], ['label' => 'Total Products', 'number' => 500, 'suffix' => '+']]); @endphp

                        <div id="stats-wrapper">
                            @foreach ($stats as $s)
                                <div class="row mb-2 align-items-center">
                                    <div class="col-md-5">
                                        <input type="text" name="stats_label[]" class="form-control"
                                            value="{{ $s['label'] ?? '' }}" placeholder="Label">
                                    </div>
                                    <div class="col-md-3">
                                        <input type="number" name="stats_number[]" class="form-control"
                                            value="{{ $s['number'] ?? 0 }}" placeholder="Number">
                                    </div>
                                    <div class="col-md-2">
                                        <input type="text" name="stats_suffix[]" class="form-control"
                                            value="{{ $s['suffix'] ?? '' }}" placeholder="Suffix">
                                    </div>
                                    <div class="col-md-2">
                                        <button type="button" class="btn remove-stat">Remove</button>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                                ...
                                @foreach ($team as $member)
                                    <div class="row">
                                        <div class="col-md-5 mb-3">
                                            ...
                                            <input type="hidden" name="existing_team_image[]"
                                                value="{{ $member['image'] ?? '' }}">
                                        ...
                                        <div class="col-md-2 mb-3 d-flex justify-content-center align-items-end">
                                            <button type="button" class="btn remove-team">Remove</button>
                                        </div>
                                    </div>
                                @endforeach
                            @else
            ...
            <div class="mt-4">
                <div class="text-left">
                    <button class="btn btn-lg" type="submit"><i class="fa fa-check-circle me-2"></i>Save About</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        document.getElementById('add-why').addEventListener('click', function() {
            ...
            node.innerHTML =
                `<input type="text" name="why_list[]" class="form-control me-2" placeholder="Bullet point"><button type="button" class="btn remove-why">Remove</button>`;
        ...
        document.getElementById('add-stat').addEventListener('click', function() {
            ...
            <div class="col-md-2"><button type="button" class="btn remove-stat">Remove</button></div>
        </div>`;
        ...
        document.getElementById('add-team').addEventListener('click', function() {
            const html = `<div class="team-row row">
                <div class="col-md-5 mb-3">
                    <label>Photo</label><br>
